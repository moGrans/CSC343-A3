...
CREATE VIEW valid_resv AS
    SELECT cr.email, cr.reservation_id
...
CREATE VIEW num_shared AS
    SELECT vr.email, count(vr.reservation_id) AS num_share_reservation
...
INSERT INTO q2 
	SELECT c.name, c.email, tt.num_share_reservation
	FROM top_two tt join Customer c
            ON tt.email = c.email
    ORDER BY tt.num_share_reservation [DESC], c.email;